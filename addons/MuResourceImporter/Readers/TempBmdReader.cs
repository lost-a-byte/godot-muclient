...
public class TempBMDReader : Client.Data.BaseReader<BMD>
{
    ...
    {
        if (version is not 12 and not 14 and not 15) return;

        var size = BitConverter.ToInt32(buffer, 4);
        var enc = new byte[size];
        Array.Copy(buffer, 8, enc, 0, size);

        var dec = version switch
        {
            12 => FileCryptor.Decrypt(enc),
            14 => ModulusCryptor.Decrypt(enc),
            15 => LEACrypto.Decrypt(enc),
            _ => throw new NotImplementedException("BMD Version " + version + "Not supported!")
        };
        if (version == 14)
        {
            size -= 34;
